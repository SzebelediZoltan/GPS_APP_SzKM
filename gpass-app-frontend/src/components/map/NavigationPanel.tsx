...
import { useRouting, useNominatimSearch, type NominatimResult } from "@/hooks/map/useRouting"
...
function ResultList({ results, searching, query, onSelect }: {
  ...
  return (
    <div className="rounded-xl border border-border bg-background/95 backdrop-blur overflow-hidden">
      ...
      {!searching && results.length === 0 && (
        <div className="px-3 py-2.5 text-sm text-muted-foreground">Nem található ilyen hely.</div>
      )}
      {!searching && results.map((r) => (
        <button
          key={r.place_id}
          onClick={() => onSelect(r)}
          className="w-full flex items-start gap-2 px-3 py-2.5 text-left text-sm hover:bg-muted transition border-b border-border/50 last:border-0"
        >
...
export default function NavigationPanel({ currentPosition, onOpenMobile }: PanelProps) {
  ...
  const handleSelect = async (r: NominatimResult) => {
    ...
    const routes = await planRoute({ start: currentPosition, end: destination })
    if (routes.length > 0) {
      setPreview(routes)
      ...
      <div className="absolute top-4 left-4 z-1000 hidden md:block">
        {!isDesktopOpen && (
          <Button
            ...
            className="w-11 h-11 rounded-xl bg-card text-foreground border border-border shadow-md flex items-center justify-center hover:bg-muted active:scale-95 transition cursor-pointer"
          >
        ...
        {isDesktopOpen && (
          <div className="w-[min(20rem,calc(100vw-2rem))] rounded-xl border border-border bg-background/95 backdrop-blur shadow-xl p-4 space-y-3 animate-in fade-in-0 zoom-in-95 duration-200">
            <div className="relative">
              ...
              />
              {query && (
                <button onClick={reset} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                  <X className="w-3.5 h-3.5" />
                </button>
              )}
            </div>

            <ResultList results={results} searching={searching} query={query} onSelect={handleSelect} />
