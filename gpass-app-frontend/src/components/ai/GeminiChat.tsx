...
export function GeminiChat({ className = '' }: GeminiChatProps) {
  const { loading, error, sendMessage, conversationHistory, clearHistory, resetError } =
    useOpenRouter()
    ...
    <div className={`flex flex-col h-full gap-4 ${className}`}>
          ...
          {conversationHistory.length === 0 && !loading && (
            <div className="h-full flex items-center justify-center text-center">
              <div className="text-muted-foreground">
                ...
                <p className="text-xs mt-1 opacity-75">Írj egy kérdést az alul lévő mezőbe</p>
              </div>
            </div>
          )}
      ...
      <form onSubmit={handleSendMessage} className="flex gap-2">
        <Input
          ...
          className="flex-1"
        />
        <Button type="submit" disabled={loading} size="icon" title="Üzenet küldése">
